...
(function (global, factory) {
    ...
    class DjiGeozones extends Control {
                ...
                return new Promise((resolve) => {
                    setTimeout(async () => {
                        if (request !== idInfoRequest)
                            return;
                        const center4326 = proj_js.transform(coordinate, this._projection, 'EPSG:4326');
                        const clickLatLng = {
                            lat: center4326[1],
                            lng: center4326[0]
                        };
        ...
        _getInfoFromView(clear = false) {
            ...
            setTimeout(async () => {
                ...
                try {
                    ...
                    const center4326 = proj_js.transform(center, this._projection, 'EPSG:4326');
                    const viewLatLng = {
                        lat: center4326[1],
                        lng: center4326[0]
                    };
        ...
        async _getApiGeoData(typeApiRequest, latLng) {
            ...
            const getAreas = async (centerLatLng, searchRadius) => {
                ...
                if (this._options.buffer) {
                    ...
                    extent = proj_js.transform(extent, this._projection, 'EPSG:3857');
                    // Apply buffer in meters
                    extent = this._options.buffer
                        ? extent_js.buffer(extent, this._options.buffer)
                        : extent;
                    // Restore extent in map projection
                    extent = proj_js.transform(extent, 'EPSG:3857', this._projection);
                }
                const polygon = Polygon.fromExtent(extent);
                if (this._areaDownloaded) {
                    if (this._areaDownloaded.intersectsCoordinate(extent_js.getCenter(extent)) &&
                        this._areaDownloaded.intersectsCoordinate(extent_js.getBottomLeft(extent)) &&
                        this._areaDownloaded.intersectsCoordinate(extent_js.getTopRight(extent)) &&
                        this._areaDownloaded.intersectsCoordinate(extent_js.getBottomRight(extent)) &&
                        this._areaDownloaded.intersectsCoordinate(extent_js.getTopLeft(extent))) {
            ...
            const getMapRadius = ({ lng, lat }) => {
                ...
                extent = proj_js.transformExtent(extent, this._projection, 'EPSG:4326');
                const posSW = [extent[0], extent[1]];
                const centerToSW = sphere_js.getDistance(center, posSW) + this._options.buffer;
        ...
        static colorWithAlpha(color, alpha = 1) {
            const [r, g, b] = Array.from(color_js.asArray(color));
